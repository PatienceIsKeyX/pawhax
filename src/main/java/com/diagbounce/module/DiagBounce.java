package com.diagbounce.module;

import baritone.api.BaritoneAPI;
import baritone.api.pathing.goals.GoalBlock;
import com.diagbounce.DiagBounceAddon;
...
public class DiagBounce extends Module {
    ...
    public DiagBounce() {
        super(DiagBounceAddon.CATEGORY, "diag-bounce", "High-speed diagonal elytra bounce.");
    }
    ...
    private void tickAligning() {
        ...
        if (mc.player.horizontalCollision) {
            ...
            if (wasAlignedCollision) {
                ...
                if (collisionTicks > 0) dbg("align: brief collision " + collisionTicks + "t then lost (stuckTimer=" + stuckTimer + ")");
                collisionTicks = 0;
            }
        }
    ...
    private void enterBouncing() {
        ...
        capturedTargetY = (int) Math.floor(mc.player.getY());
        lastPos = mc.player.getPos();
        stuckTimer = 0; bouncingTicks = 0; zeroSpeedTicks = 0;
        rubberBandCount = 0; driftTicks = 0; postSlipTicks = 0;
        preBounceDelay = 5; postPasserCooldown = 40;
        wasCruising = false; collisionTicks = 0; wasAlignedCollision = false;
    ...
    private void tickBouncing() {
        ...
        if (hspeed >= BOOST_ENGAGE_SPEED) {
            zeroSpeedTicks = 0; lowBoostTicks = 0; bouncingTicks = 0;
            state = State.BOOSTING; return;
        }
        if (wasCruising) lowBoostTicks++;
        if (lowBoostTicks > 40) {
            dbg("obstacle: stuck low speed bps=" + String.format("%.1f", hspeed));
            lowBoostTicks = 0; engageObstaclePasser(); return;
        }
        bouncingTicks++;
        if (hspeed < 1.0) zeroSpeedTicks++; else zeroSpeedTicks = 0;
        ...
        if (bouncingTicks > 200) { dbg("obstacle: bouncing timeout " + bouncingTicks + "t"); engageObstaclePasser(); return; }
        if (zeroSpeedTicks > 40) { dbg("obstacle: zero speed " + zeroSpeedTicks + "t in BOUNCING"); engageObstaclePasser(); return; }
    ...
    private void tickBoosting() {
        ...
        if (!prevOnGround && onGround)
            dbg("dbg land: bps=" + String.format("%.1f", hspeed) + " vel=(" + String.format("%.2f", vel.x) + "," + String.format("%.2f", vel.y) + "," + String.format("%.2f", vel.z) + ") y=" + String.format("%.2f", mc.player.getY()));
        if (prevOnGround && !onGround)
            dbg("dbg takeoff: bps=" + String.format("%.1f", hspeed) + " velY=" + String.format("%.2f", vel.y) + " gliding=" + mc.player.isGliding());
        prevOnGround = onGround;
        if (onGround && hspeed < speed.get()) mc.player.jump();
        if (hspeed < 1.0) zeroSpeedTicks++; else zeroSpeedTicks = 0;
        if (hspeed >= 40.0) wasCruising = true;
        if (hspeed < BOOST_ENGAGE_SPEED) { if (wasCruising) lowBoostTicks++; } else lowBoostTicks = 0;
        if (mc.player.age % 40 == 0) {
            double drift = distancePointToDirection(mc.player.getPos(), yawToDirection(lockedYaw), activationPos);
            dbg("dbg boost: bps=" + String.format("%.1f", hspeed) + " drift=" + String.format("%.1f", drift) + " lowBoost=" + lowBoostTicks + " zero=" + zeroSpeedTicks + " y=" + mc.player.getBlockPos().getY());
        }
        if (zeroSpeedTicks > 40) { dbg("obstacle: zero speed " + zeroSpeedTicks + "t in BOOSTING"); engageObstaclePasser(); return; }
        if (lowBoostTicks > 20) { dbg("obstacle: below boost speed for " + lowBoostTicks + "t"); engageObstaclePasser(); return; }
        if (postPasserCooldown > 0) postPasserCooldown--;
        else if (shouldPassObstacle()) engageObstaclePasser();
    }

    private void tickObstaclePassing() {
        if (!baritoneLoaded) { state = State.ALIGNING; return; }
        if (prePasserDelay > 0) { prePasserDelay--; return; }
        ...
        if (obstaclePassingTicks > 600) {
            ...
            BaritoneAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess().setGoal(null);
            holdForwardKey(false);
            stuckTimer = 0; zeroSpeedTicks = 0; lowBoostTicks = 0; wasCruising = false;
            collisionTicks = 0; wasAlignedCollision = false; obstaclePassingTicks = 0;
            settleTicksRemaining = 5; state = State.ALIGNING; return;
        }
        if (BaritoneAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess().getGoal() == null) {
            ...
            stuckTimer = 0; zeroSpeedTicks = 0; lowBoostTicks = 0; wasCruising = false;
            collisionTicks = 0; wasAlignedCollision = false; obstaclePassingTicks = 0;
            settleTicksRemaining = 5; state = State.ALIGNING;
        }
    }
    ...
    private boolean shouldPassObstacle() {
        ...
        double drift = distancePointToDirection(mc.player.getPos(), yawToDirection(lockedYaw), activationPos);
        if (drift > 3.0) driftTicks++; else driftTicks = 0;
    ...
    private void engageObstaclePasser() {
        ...
        obstaclePassingTicks = 0; prePasserDelay = 10; pendingPasserGoal = null;
        ...
        while (currDistance <= MAX_OBSTACLE_RANGE) {
            ...
            if (xSolid || zSolid) {
                ...
                pendingPasserGoal = candidate; state = State.OBSTACLE_PASSING; return;
            }
        ...
        pendingPasserGoal = fallbackGoal; state = State.OBSTACLE_PASSING;
    }

    private void dbg(String msg) { if (debugMessages.get()) info(msg); }
    ...
    private void releaseForwardKey() { holdForwardKey(false); }
    ...
    private static double distancePointToDirection(Vec3d point, Vec3d direction, Vec3d start) {
        point = point.multiply(1, 0, 1); start = start.multiply(1, 0, 1); direction = direction.multiply(1, 0, 1);
        double lenSq = direction.lengthSquared();
        if (lenSq == 0) return 0;
        Vec3d diff = point.subtract(start);
        double proj = diff.dotProduct(direction) / lenSq;
        return diff.subtract(direction.multiply(proj)).length();
    }
}
